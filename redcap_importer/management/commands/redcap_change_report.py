...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if project_response["is_longitudinal"] != oProject.is_longitudinal:
            ...
            output.append(
                "Longitudinality of this project has changed from {} to {}".format(
        ...
        if field_name_response[0]["export_field_name"] != oProject.primary_key_field:
            has_major_project_change = True
            output.append(
                "The project primary key field has changed from {} to {}".format(
        ...
        for entry in response:
            ...
            oInstrument = models.InstrumentMetadata.objects.filter(
                project=oProject, unique_name=entry["instrument_name"]
            ).first()
            if not oInstrument:
        ...
        if oProject.is_longitudinal:
            ...
            for entry in response:
                ...
                oArm = models.ArmMetadata.objects.filter(
                    project=oProject, arm_number=int(entry["arm_num"])
                ).first()
                if not oArm:
                    output.append("New arm: {}".format(entry["arm_num"]))
        ...
        for entry in response:
            oInstrument = models.InstrumentMetadata.objects.filter(
                project=oProject, unique_name=entry["form_name"]
            ).first()
            if not oInstrument:
            ...
            if django_data_type:
                ...
                if not oField:
                    output.append(
                        "New field: {}.{}".format(oInstrument.unique_name, entry["field_name"])
                    )
                else:
                    # check if lookup values have changed
                    if json.dumps(field_display_lookup) != oField.field_display_lookup:
                        output.append(
                            "Field '{}' lookup values have changed".format(
                                oInstrument.unique_name, entry["field_name"]
                            )
                        )
            ...
            if oInstrument.unique_name in loaded_instruments:
                for oField in models.FieldMetadata.objects.filter(instrument=oInstrument):
                    if not oField.unique_name in loaded_fields:
                        output.append(
                            "Field removed: {}.{}".format(
                                oInstrument.unique_name, oField.unique_name
                            )
                        )
